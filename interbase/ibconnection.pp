...
uses
  Classes, SysUtils, easyDB, db, dbconst, bufdataset,
...
type
  ...
  private
    ...
    procedure SetParameters(cursor : TSQLCursor; aTransation : TEDBTransaction; AParams : TParams);
...
procedure TIBConnection.PrepareStatement(cursor: TSQLCursor;ATransaction : TEDBTransaction;buf : string; AParams : TParams);
...
procedure TIBConnection.Execute(cursor: TSQLCursor;atransaction:TEDBTransaction; AParams : TParams);
var tr : pointer;
    out_SQLDA : PXSQLDA;
begin
  tr := aTransaction.Handle;
...
procedure TIBConnection.SetParameters(cursor : TSQLCursor; aTransation : TEDBTransaction; AParams : TParams);
...
procedure TIBConnection.LoadBlobIntoBuffer(FieldDef: TFieldDef;ABlobBuf: PBufBlobField; cursor: TSQLCursor; ATransaction : TEDBTransaction);
const
